...
interface ISelectProps {
  ...
  multiCheck?: boolean
  list?: boolean
  inputType?: 'input' | 'button' | 'inputCustom'
  placeholder?: string
...
<template>
  <div class="block" :class="width ?? 'w-full'">
    ...
    <Dropdown :close-on-select="multi ? true : closeOnSelect" :content-class="'overflow-inherit'" my-2
      :get-label="getLabel" :get-value="getValue">
      <template v-slot="{ events }">
        ...
        <div v-else-if="inputType === 'input'" id="dropdown-input-trigger" v-on="events" justify-between
          class="input p-2" :class="optionClass ?? 'input-option-class'" name="dropdown-input-trigger">
          {{ hasValue ? valueText : (placeholder ?? 'Select') }}
          <div i-mdi:menu-down class="w-6" />
        </div>

        <FormTextInput v-else-if="inputType ==='inputCustom'"  :id="id" v-on="events" justify-between
          class="input p-2" :class="optionClass ?? 'input-option-class'" :name="name" :label="label"
      >
          <template #default>
      ...
      <template #content="{ attributes, actions}">
        <slot name="select-content">
          <button v-for="(option, idx) in options" :key="idx" v-bind="attributes"
            class="dropdown-item flex flex-col w-full " @click="handleSelection(option);actions.close()">
            <FormCheckbox v-if="multiCheck" class="font-xs" :value="getValue(option)"
...
.input-option-class {
  @apply text-color-primary border border-color w-full background-secondary rounded-lg;
}
</style>
