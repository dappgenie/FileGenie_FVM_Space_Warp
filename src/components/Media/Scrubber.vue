...
<template>
  <div ref="scrubber" class="relative h-2 rounded cursor-pointer select-none bg-black dark:bg-white dark:bg-opacity-10 bg-opacity-20" @mousedown="scrubbing = true">
    <div class="relative overflow-hidden h-full w-full rounded">
      <div class="h-full absolute opacity-30 left-0 top-0 bg-[##1285b6] w-full rounded" :style="{ transform: `translateX(${secondary / max * 100 - 100}%)` }" />
      <div class="relative h-full w-full bg-[#1285b6] rounded" :style="{ transform: `translateX(${value / max * 100 - 100}%)` }" />
    </div>
